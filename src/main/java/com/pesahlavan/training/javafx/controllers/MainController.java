package com.pesahlavan.training.javafx.controllers;

import com.pesahlavan.training.javafx.entity.Person;
import com.pesahlavan.training.javafx.fxml.EditView;
...
//                    showDialog();
    ...
    private void fillTable() {
        if (txtSearch.getText().trim().length() == 0) {
            page = (Page) addressBook.findAll(0, PAGE_SIZE);
        }else {
    ...
    private void fillTable(int pageNumber) {
        if (txtSearch.getText().trim().length() == 0) {
            page = (Page) addressBook.findAll(pageNumber, PAGE_SIZE);
        }else {
